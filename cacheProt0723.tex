...
\twoSpaces \alt indexTransvar indTranspolicy (Para v i)=Para (indexTransval indTranspolicy i) (indTranspolicy i)\\
\twoSpaces \alt  indexTransvar indTranspolicy (Field v str)=Field (indexTransval~indTranspolicy v) str\\

letrec indexTransval indTranspolicy (enum strt strv)= enum strt strv\\
\twoSpaces \alt indexTransval indTranspolicy (index ind)=index (indTranspolicy i)\\
...
We write {\tt str$_j$ for $\mathsf{Const}~(\mathsf{index}~ \mathsf{indexTrans}~str~i)$} in the remaining parts of this work, where {\tt str} is a string:
...
actual formulas( rules) of $N-$parameterized model in Isabelle syntax. Besides, if a formula such as ? contains more than one index, we requires that indice should be mutually different. In order to  define the mutual difference between parameters of an invariant (or a rule), we need a function $\mathsf{comb}~m~n$ to compute a combination of $n$ elements from $n$ elements: $C_m^{n}$. For instance, $C_3^{2}=[[1,2],[1,3],[2,3]]$.
...
\begin{specification}
let neqL L =
\twoSpaces  map ($\lambda$ j. $\neg_C$ ( iInv$_j=_C$  iR$_i))$   L\\

let indexCondGen nI  j i=\\
\twoSpaces  if (j $\le$ nI) then   (iInv$_j$$=_C$iR$_i$)\# (map neqL ((1 upto nI) subtract j) i))\\
\twoSpaces  else   (map neqL  (1 upto nI) i)\\
...
Given a list of paramters {\tt rparas} to instantiate a rule, {\tt caseGen lenInv rparas} returns the assumptions on the case analysis of the rule parameters and invariant parameters, for instance,  {\tt caseGen 2 [1]=[[iInv$_1$$=_C$iR$_1$],[$\neg_C$iInv$_2$$=_C$iR$_1$]]}; {\tt caseGen 2 [1,3]=[[iInv$_1$$=_C$iR$_1$,$\neg_C$ iInv$_1$$=_C$iR$_2$], [$\neg_C$iInv$_2$$=_C$iR$_1$,$\neg_C$iInv$_2$$=_C$iR$_2$]]}. {\tt find\_first~ ($\lambda$ j. i=j) rparas} returns the index of the first element which is equal to $i$  {\tt lookup} is another parameter symbolic transformation policy: if {\tt $\le$lenInv}, then {\tt i} will be transformed into  iInv$\_{i}$, else return iR$\_{j}$ where $j$ is the index of $i$ in the list {\tt rparas}.
...
Now we formally define the generalization from a concrete case to a parameterized case, there are three generalization principles:
 ...
 \item[(1)] For a normalized concrete invariant formula  $cinv$ computed by {\sf invFinder}, $\mathsf{proofGen}$   generates a  record  {\tt symbForm \{name::string\} \{formFld::formula\} \{paraNumFld::nat\}  \{gFormFld::formula\}}, where field  {\tt name}  stores a string for the name of this invariant, formFld  symbolic formula $pInv=\mathsf{indexTrans2Form}~"i"~$, paraNumFld the number of formal parameters of this invariant $NP=\mathsf{paraNumOfInv}~pInv$, and gFormFld a formula {\tt  diffConGen~NP~i} to specify a
     ...
     $\mathsf{proofGen}$ creates a table $symbInvs$ to store all information of the symbolic invariant formulas.
 ...
 \item[(2)] $\mathsf{proofGen}$   generates  a  record  {\tt symbRule \{name::string\} \{ruleFld::rule\} \{paraNumFld::nat\}  \{gFormFld::rule\}} to store information on a symbolic rule, where field  {\tt name}  stores a string for the name of this rule, ruleFld  stores a  symbolic rule $prule$, which is translated from a rule in syntax in section \ref{sec:protocolSyntax}, and  paraNumFld the number of formal parameters of this symbolic rule $NP=\mathsf{paraNumOfRule}~pRule$,  and gFormFld a formula {\tt  diffConGen~NP~iR} to specify a
     side constraint on the parameters of this invariant with this generalization.  $\mathsf{proofGen}$ creates a table $symbRules$ to store all information of the symbolic invariant rules.%
 ...
 \item[(3)]For an item {\tt (r,cinv,rparas,holdtype,f')} in a line of a causal table, suppose $cr=pr ~rparas$, $\mathsf{proofGen}$ generates  a  record  {\tt symbLine \{ruleName::string\} \{invName::string\} \{caseFld::formula\} \{holdType::invHoldTagType} \{hold3Form:formula option\}. Here the filed caseFld is generated by $\mathsf{caseGen}~ lenInv~ rparas$   to specify the case condition between parameters of the invariant and rule, which is generalized from the causal line. The filed holdType is to indicate which causal relation are. If {\tt holdType=invHold3}, then {\tt hold3Form={\sf SOME} (indexTransForm (lookup lenInv   rparas) f')}, else {\tt hold3Form={\sf NONE}}. $\mathsf{proofGen}$ creates a table $symbCausalTab$ to store all information of the symbolic causal relation.
 \end{description}
%-------------------------------------------------------------------------
...
\begin{specification}
1lemma critVsinv1:\\
2  assumes  a1: iR1 $\le$ N and a2: iInv1 $\le$ N and a3: iInv2 $\le$ N and a4: iInv1 $\neq$ iInv2\\
3  shows  invHoldForRule (inv1 iInv1 iInv2) (crit iRule) (invariants
  N)\\
4  proof -\\
...
8  \twoSpaces then have invHoldForRule (inv1 iInv1 iInv2) (crit iR1)
(invariants
  N)
by auto\}\\
...
11 \twoSpaces then have invHoldForRule (inv1 iInv1 iInv2) (crit iR1) (invariants
  N)
by auto\}\\

12   moreover\{assume  b1:(iR1 $\ne$  iInv1 ¡Ä   iR1 $\ne$  iInv2)\\
13 \twoSpaces have invHoldForRule2 (inv1 iInv1 iInv2) (crit iR1) (invariants N)\\
  \twoSpaces \twoSpaces  proof( auto) qed\\
14 \twoSpaces then have invHoldForRule (inv1 iInv1 iInv2) (crit iR1)
(invariants
  N)
by auto\} \\

15ultimately show invHoldForRule (inv1 iInv1 iInv2) (crit iR1)
(invariants N)\\
16qed\\
\end{specification}
...
4  proof -\\
   ...
   by blast\\
   from a2 obtain iInv1 iInv2 where a2:" iInv1 $\le$ N $\wedge$ iInv2 $\le$ N $\wedge$ iInv1 $\neq$ iInv2 $\wedge$ f=inv1  iInv1 iInv2"\\
   by blast \\
5  have iR1=iInv1 $\vee$ iR1=iInv2 $\vee$ (iR1 $\ne$ iInv1 $\wedge$  iR1 $\ne$ iInv2) by auto\\
...
let symbCausalRec2DisjItem symbRec=\\
...
\twoSpaces let asms=allNamedAsmsGen lenPInv lenPRule ruleName inVName symbRules symbInvs in\\
...
%(invariants
 % N)  \\
\twoSpaces  \%s \\
\twoSpaces ultimately show invHoldForRule (\%s \%s) (\%s \%s)(invariants
  N)\\
\twoSpaces qed"\\

\twoSpaces (ruleName inVName,  \\
\twoSpaces asms, \\
\twoSpaces invName,invParasGen lenPInv, ruleName, rulePrarasGen lenPRule,\\
...
next   show  $\forall$inv r. inv $\in$ invariants N$\longrightarrow$
 r $\in$rules N$\longrightarrow$invHoldForRule inv r (invariants N) \\
   ...
   proof((rule allI)+,(rule impI)+)\\
\twoSpaces      fix inv r \\
\twoSpaces         assume b1: inv $\in$ invariants N  and b2:r $\in$ rules N\\

\twoSpaces      have c1:ex1 N ($\lambda$i.r=exit i)$\vee$ex1 N
($\lambda$i.r=idle i)
  $\vee$ ex1P N ($\lambda$i.r=crit i)$\vee$ \\
\twoSpaces ex1P N ($\lambda$i.r=try i)
\twoSpaces\twoSpaces         by(cut$\_$tac  b2,auto)\\
\twoSpaces moreover\\
\twoSpaces\oneSpace $\{$assume c1: ex1P N ($\lambda$i.  rule=exit i) \\
\twoSpaces\oneSpace from c1 obtain iR where c2: iR $\le$ N $\wedge$ r= Exit iR\\

\twoSpaces\twoSpaces         by (auto simp add: ex1P$\_$def)\\

\twoSpaces\oneSpace have c3:   exTwoLessP N ($\lambda$i j. inv=inv1 i j)$\vee$ex1P N   ($\lambda$i.inv inv2 i)\\
\twoSpaces\oneSpace$\vee$ exTwoLessP N ($\lambda$ i j.inv=inv3 i j) $\vee$ ex1P N  ($\lambda$ i.  inv=inv4 i)$\vee$ exTwoLessP N($\lambda$
 i j. inv=inv5 i j)\\
\twoSpaces\twoSpaces   ~by (cut$\_$tac b1, simp )\\

\twoSpaces\oneSpace  moreover\\
\twoSpaces\oneSpace\twoSpaces$\{$assume d1: ex2P N ($\lambda$ i j.  inv= inv1 i j)\\

\twoSpaces\oneSpace\twoSpaces from d1 obtain i1 and i2 where d2: i1 $\le$ N $\wedge$ i2 $\le$
N $\wedge$ i1 $\neq$i2$\wedge$  inv= inv1 i1 i2\\
\twoSpaces\twoSpaces\twoSpaces            by (-,unfold ex2P$\_$def,auto)\\
\twoSpaces\oneSpace \twoSpaces  have inv r (invariants N) \\
\twoSpaces\twoSpaces\twoSpaces         by(cut$\_$tac  c2 d2, metis exitVsInv1)\}\\
\twoSpaces\twoSpaces\twoSpaces  ......\\
\twoSpaces\oneSpace\twoSpaces moreover\\
\twoSpaces\oneSpace\twoSpaces      $\{$assume d1: ex2P N ($\lambda$ i j.  inv= inv5 i j)\\

\twoSpaces\oneSpace\twoSpaces ......~~~~          \\
\twoSpaces\oneSpace \twoSpaces  have invHoldForRule inv r (invariants N) \\

\twoSpaces\twoSpaces\twoSpaces     by(cut$\_$tac c2 d2, simp) $\}$\\


\twoSpaces\oneSpace \twoSpaces ultimately have invHoldForRule inv r (invariants N) \\

\twoSpaces\twoSpaces\twoSpaces     by blast \\%\}\\
\twoSpaces\twoSpaces\twoSpaces        $\}$\\

\twoSpaces  moreover\\
\twoSpaces\oneSpace  $\{$assume c1: ex1P N ($\lambda$i.  rule= Idle i)\\

\twoSpaces\oneSpace \oneSpace  ......\\

\twoSpaces\oneSpace \oneSpace   \}\\
\twoSpaces                   ultimately show invHoldForRule inv r (invariants N) ~~by blast\\
qed\\
qed\\
...
\wedge i\ne k \wedge j\ne k \wedge P~ i~j~k)$. Line 6 starts the case analysis on
...
let parasStr=invParasGen len in\\
sprintf "
moreover\\
\twoSpaces\oneSpace\twoSpaces$\{$assume d1: \%s)\\

\twoSpaces\oneSpace\twoSpaces from d1 obtain \%s where d2: \%s$\wedge$  inv= \%s \%s\\
\twoSpaces\twoSpaces\twoSpaces            by (auto)\\
\twoSpaces\oneSpace \twoSpaces  have inv r (invariants N) \\
\twoSpaces\twoSpaces\twoSpaces         by(cut$\_$tac  c2 d2, metis \%sVs\%s)\}"\\
(frmDisj symbInvs invName,\\
 parasStr,constraint,invName,parasStr,\\
 ruleName, invName);\\
...
\twoSpaces\oneSpace from c1 obtain \%s where c2: \%s $\wedge$ r= \%s \%s\\

\twoSpaces\twoSpaces         by (auto  )\\

\twoSpaces\oneSpace have c3:   \%s\\
\twoSpaces\twoSpaces   ~by (cut$\_$tac b1, simp )\\
\twoSpaces\oneSpace ultimately have invHoldForRule inv r (invariants N) \\

\twoSpaces\twoSpaces\twoSpaces     by blast \\%\}\\
\twoSpaces\oneSpace        $\}$\\
(ruleDisj symbRules ruleName,\\
 parasStr,constraint,ruleName,parasStr,\\
 frmsDisjs symbInvs,   \\
