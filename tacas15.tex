...
The aim of this paper is to apply {\sf paraVerifier} to parameterized verification of FLASH protocol. {\sf paraVerifier} can be regarded
as both an invariant finder and  a formal proof generator. Given an invariant $inv$ such as a mutual-exclusion proerty, and a rule set $rs$ in MURPHI which represents a protocol,
...
work lies the following aspects: (1) {\sf paraVerifier} is guided by
...
cache coherence protocol instance {\tt prot.m}, is compiled into an internal form {\sf prot.ml}, then fed into the
...
protocol instance. This step solve the first  problem (1).
 A table {\tt prot.tbl} is worked out  to store the set of ground invariants and
