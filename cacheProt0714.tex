...
let proof3Gen ruleName invName f=\\
\twoSpaces sprintf\\
\twoSpaces "moreover\{assume  b1:%\s\\
\twoSpaces have invHoldForRule3 (\%s \%s) (\%s \%s) (invariants N)\\
\twoSpaces  \twoSpaces   proof(simp, rule\_tac x=\%s  in exI,auto)qed\\
\twoSpaces \twoSpaces then have invHoldForRule (\%s \%) (\%s \%s)
(invariants
  N)
\twoSpaces by auto"\\
\twoSpaces (form2Isabelle caseF, \\
\twoSpaces invName,invParasGen lenPInv, ruleName, rulePrarasGen lenPRule,\\
\twoSpaces form2Isabelle f,\\
\twoSpaces invName,invParasGen lenPInv, ruleName, rulePrarasGen lenPRule)\\
let symbCausalRec2Proof symbRec=\\
\twoSpaces  val (symbLine rnmae invName caseF holdTag formOpt)=symbRec in\\
\twoSpaces     if (holdTag=invHold1) then \\
\twoSpaces      proof1Gen rnmae invName caseF\\
\twoSpaces     else if  (holdTag=invHold2) then \\
\twoSpaces      proof2Gen rnmae invName caseF\\
\twoSpaces     else proof3Gen rnmae invName caseF\\
...
\twoSpaces       have c1:exLessP N ($\%$i.r=exit i)$\vee$exLessP N
($\%$i.r=idle i)
  $\vee$ exLessP N ($\%$i.r=crit i)$\vee$ \\
\twoSpaces exLessP N ($\%$i.r=try i)
...
study. First we recall our paper proof. Here, it is a typically
