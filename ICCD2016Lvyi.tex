...
%\IEEEauthorblockA{State Key Laboratory of Computer Science, Institute of Software, Chinese Academy of sciences}
%\thanks{%
...
%Another novel
%feature of our method lies in that \redt{the correctness of a
%protocol is guaranteed  by a formal and readable proof which is automatically generated.}
...
%: ��if the method works on FLASH, then there is a good chance that it will also work on many real-world cache coherence protocols�� \cite{Chou2004}. However, the existing approaches, which have verified FLASH, need too much human intervention. The first full verification of safety properties of FLASH is done by work in \cite{Park1996a}. Park and Dill  proved the safety properties of FLASH using  PVS \cite{cade92-pvs}. %They introduce the aggressioned FLASH protocol, which
...
%However, theorem proving in a theorem prover like Isabelle is   interactive, not automatic.
...
%\vspace{-0.2cm}
\begin{itemize}%[leftmargin=0pt,noitemsep,nolistsep]
...
\times rule ~set \Rightarrow bool$.
 $consistent( invs,inis, rs)$ holds if the following conditions hold: (1) for all formulas $inv\in invs$ and $ini\in inis$ and all states $s$, $s \models ini$ implies $s \models inv$; (2) for all formulas $inv\in invs$ and rules  $r \in rs$ and all states $s$,  $\mathsf{invHoldRule}(s, inv, r, invs   )$.
...
Theorem \ref{consistentLemma} is our main tool to apply to prove. Let us recall the proof goal set in Example \ref{example1}:  the mutual exclusion property holds for each reachable state of the mutual-exclusion protocol. %Let us recall example \ref{example1} and  \ref{example2}.
...
which is a factor to specify a case by comparing $LR_{[i]}$ with $LI_{[j]}$. This will be the key idea used to  generalize the concrete  invariants and causal relations computed by {\sf invFinder}. } %All the auxiliary invariant formulas will be in a normalized form, and checked with any parameterized rule whether some kind of causal relation hold between them.
%{\bf Our Parameter Instantiation Policy:}
Let $cinv$  be a normalized concrete invariant w.r.t. a parameterized invariant $pinv$, $pr$ be a parameterized rule, $m$ be the number of actual parameters occurring in $cinv$, and $n$  be the number of formal parameters occurring in $pr$,  our policy is to compute a quotient of $\mathsf{perms}_{m}^{n}$, denoted as $\mathsf{cmpSemiperm}(m+n,n )$, and use its elements as a group of parameters to instantiate $pr$ into a set $crs$ of concrete rules.\footnote{the details of computing $\mathsf{cmpSemiperm}(m+n,n )$ can be found in \cite{LiCache16}.}  For instance, for the invariant $\negc (n[1]\eqc C \andc n[2]\eqc C)$ (or $\mathsf{mutualInv}(1,2)$), three groups of parameters [1], [2], [3] are used to instantiate $\mathsf{crit}$ respectively. Each of the instantiation results %(\caicomment{do instantiation results mean concrete rules? would be better to keep consistent.})
...
\vspace{-5pt}
\begin{definition}
Let $LR$ be a permutation which represents a list of rule parameters,   $LI$   a  permutation which  represents a list of invariant parameters, we define:
...
After the second phase of the generalization, the symbolic cases and the according causal relations on rule $\mathsf{crit}$ and $\mathsf{mutualInv}$, which are transferred from Table \ref{table:groundCausalRelation} in  Table \ref{table:symbCausalRelation}. Here $r'=\mathsf{crit}(\iR_1)$, and $f'=\mathsf{mutualInv}(\iInv_1,\iInv_2)$. %Notice both $r'$ and $f'$ are in symbolic form here.
...
%German is moderate, and FLASH protocol is  at an industrial scale.
...
%This exploration is seldom discussed in any previous work. But these readable invariants  are really needed for the analysis of a protocol.
...
\bibitem{FLASHCache}
J. Kuskin \emph{et al},
``The Stanford FLASH multiprocessor,'' in \emph{ISCA}, 1994, pp.302--313.

\bibitem{Park1996a}
S.~Park and D.~L. Dill, ``Verification of flash cache coherence protocol by
  aggregation of distributed transactions,'' in \emph{SPAA}, 1996, pp. 288--296.

\bibitem{McMillan2001}
K.~L. McMillan, ``Parameterized verification of the flash cache
  coherence protocol by compositional model checking,'' in \emph{CHARME}, 2001, pp. 179--195.

\bibitem{Chou2004}
C.-T. Chou \emph{et al}, ``A simple method for parameterized
  verification of cache coherence protocols,'' in \emph{FMCAD}, 2004, pp.
  382--398.

\bibitem{Conchon2013}
S.~Conchon, A.~Goel, S.~Krstic, A.~Mebsout, and F.~Zaidi, ``Invariants for
  finite instances and beyond,'' in \emph{FMCAD}, 2013, pp. 61--68.

\bibitem{Pnueli1996}
A.~Pnueli and E.~Shahar, ``A platform for combining deductive with algorithmic
  verification,'' in \emph{CAV}, 1996, pp. 184--195.

\bibitem{Bjørner1997}
N.~Bj\"orner \emph{et al}, ``Automatic generation of invariants and
  intermediate assertions,'' \emph{Theoretical Computer Science}, \textbf{173}(1) 49 -- 87, 1997.
%N.~Bj\"orner, A.~Browne, and Z.~Manna, ``Automatic generation of invariants and
%  intermediate assertions,'' \emph{Theoretical Computer Science}, \textbf{173}(1) 49 -- 87, 1997.

\bibitem{Arons2001}
T.~Arons \emph{et al}, ``Parameterized verification
  with automatically computed inductive assertions.'' in \emph{CAV}, 2001, pp. 221--234.

\bibitem{Pnueli2001}
A.~Pnueli, S.~Ruah, and L.~Zuck, ``Automatic deductive verification with
  invisible invariants,'' in \emph{TACAS}, 2001, pp. 82--97.

\bibitem{Tiwari2001}
A.~Tiwari, H.~Rue\ss, H.~Sa\"idi, and N.~Shankar, ``A technique for invariant
  generation,'' in \emph{TACAS}, 2001, pp. 113--127.


\bibitem{Pandav2005}
S.~Pandav \emph{et al}, ``Counterexample guided invariant
  discovery for parameterized cache coherence verification,'' in \emph{CHARME}, 2005, pp. 317--331.

\bibitem{Lv2007}
Y.~Lv, H.~Lin, and H.~Pan, ``Computing invariants for parameter abstraction,''
  in \emph{MEMOCODE}, 2007, pp. 29--38.

\bibitem{cubicle2011}
S.~Conchon \emph{et al}, ``Cubicle: A
  parallel SMT-based model checker for parameterized systems,'' in \emph{CAV}, 2012, pp. 718--724.

\bibitem{LiCache16}
``paraVerifier,'' 2016,
  \url{https://github.com/paraVerifier/paraVerifier}.
%K.~Duan. Y.~Li, ``paraVerifier,'' 2016,
