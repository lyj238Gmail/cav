...
Intuitively, a  semi-permutation is a permutation, but two  semi-permutation $L$ and $L'$ is equivalent if and only if length of $L$ and $L'$ are the same, and  $L_i$ and   $L_i'$ are the same if $L_i \le m - n$.  Formally, we define a
 ...
 is to compute the set  $\mathsf{SemiP}_{m}^{n}$, one representative of each element will be used as a group of parameters
...
\begin{specification}
1lemma critVsinv1:\\
2  assumes  a1: iR $\le$ N and a2: iInv1 $\le$ N and a3: iInv2 $\le$ N and a4: iInv1 $\neq$ iInv2\\
3  shows  invHoldForRule (inv1 iInv1 iInv2) (crit iRule) (invariants
  N)\\
4  proof -\\
5  have iR=iInv1 $\vee$ iR=iInv2 $\vee$ (iR $\ne$ iInv1 $\wedge$  iR $\ne$ iInv2) by auto\\

6  moreover\{assume  b1:iR=iInv1\\
7  \twoSpaces have invHoldForRule3 (inv1 iInv1 iInv2) (crit iR) (invariants N)\\
 \twoSpaces  \twoSpaces   proof(simp, rule\_tac x=$\neg$ (x=true $\wedge$ n[iInv2]=C)  in exI,auto)qed\\
8  \twoSpaces then have invHoldForRule (inv1 iInv1 iInv2) (crit iR)
(invariants
  N)
by auto\}\\

9  moreover\{assume  b1:iR=iInv2\\
10 \twoSpaces have invHoldForRule3' (inv1 iInv1 iInv2) (crit iRule) (invariants N)\\
 \twoSpaces \twoSpaces   proof(simp, rule\_tac x=$\neg$ (x=true $\wedge$ n[iInv1]=C  in exI,auto)qed\\
11 \twoSpaces then have invHoldForRule (inv1 iInv1 iInv2) (crit iR) (invariants
  N)
by auto\}\\

12   moreover\{assume  b1:(iR $\ne$  iInv1 ¡Ä   iR $\ne$  iInv2)\\
13 \twoSpaces have invHoldForRule2 (inv1 iInv1 iInv2) (crit iR) (invariants N)\\
  \twoSpaces \twoSpaces  proof( auto) qed\\
14 \twoSpaces then have invHoldForRule (inv1 iInv1 iInv2) (crit iR)
(invariants
  N)
by auto\} \\

15ultimately show invHoldForRule (inv1 iInv1 iInv2) (crit iR)
(invariants N)\\
16qed\\
\end{specification}
Such an lemma is generated by collecting all the lines on the invariant {\tt inv1} and rule {\tt crit}. 
Line 2 are assumptions on the parameters of the invariant and rule, which come from three parts:  (1) the facts that all parameters of this invariant should be less than the parameter $N$; (2) the facts that all parameters of this invariant should be less than the parameter $N$; (3) the constraints of he mutual difference between parameters of the invariant (rule), which can be looked up in the field of those records  from the table $symbInvTab$ ($symbRuleTab$) by the invariant (rule) name {\tt inv1} ({\tt crit}) , which specifies that the mutual difference between two parameters. line 5 splits cases of $iR$ into all possible cases by comparing
