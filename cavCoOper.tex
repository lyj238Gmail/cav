...
Lemma \ref{lemma:simeqQuotinent} shows that, \ref{complete}) completeness of $S$ w.r.t. the set $\mathsf{perms}_{m}^{n}$ under the relation $\simeq$, \ref{distinct})  the distinct between two different elements in $S$. Therefore, $S$ has covered all analysing patterns according to the aforementioned comparing scheme between elements of $L$ with numbers $j<n-m$, and each ({\bf this sentence should be finished...}). This fact can be illustrated by the following example.
