...
%: ï¿½ï¿½if the method works on FLASH, then there is a good chance that it will also work on many real-world cache coherence protocols ï¿½ï¿½ \cite{Chou2004}. However, the existing approaches, which have verified FLASH, need too much human intervention. The first full verification of safety properties of FLASH is done by work in \cite{Park1996a}. Park and Dill  proved the safety properties of FLASH using  PVS \cite{cade92-pvs}. %They introduce the aggressioned FLASH protocol, which
...
%discovery of noninterference lemmas are all deeply influenced by McMillan ï¿½ï¿½s
%work.
...
For simplicity, a
A parameterized formula(statement, rule, and protocol) is a function $f(x_1,x_2,...,x_n)$ from a list of  to such an object. Without losing the generality, we require that parameters to instantiate a parameterized object are disjoint. For instance, $mutualInv(i,j)\equiv
  \negc (n[i]\eqc C \andc n[j]\eqc C)$ is a parameterized formula with two disjoint parameters.
   ...
   { $paras \leftarrow \mathsf{Policy}(r,cf)$;
     ...
     {$cr \leftarrow \mathsf{apply}(r,para)$;
       ...
       \If{$newInvOpt \neq NONE$}
        ...
        $newInvs\leftarrow enqueue(newInvs,newInv)$\;
...
\item $\mathsf{semiP}(m,n,S)\equiv (\forall  para \in \mathsf{perms}_{m}^{n} \exists  para' \in S. para \simeq_m^n para' ) \wedge (\forall  para\in S. \forall para'\in S. para \neq para' \longrightarrow \neg  (para \simeq_m^n para' )$.   A set $S$ is called a quotient of the set $\mathsf{perms}_{m}^{n}$ under the relation $\simeq_m^n$ if    $\mathsf{semiP}(m,n,S)$.
...
Recall that the number of actual parameters occurring in $cf$ is $n$, and that of formal parameters occurring in $r$ is $n'$,
\begin{enumerate}
\item $caseGen([1],2,1)=\iR_{1}=\iInv_1$
\item $caseGen([3],2,1)=\bigwedge^2_{j=1}\iR_{i} \neq \iInv_j)$
\item  $caseGen([1,3],2,1)=\iR_{1}=\iInv_1\bigwedge^2_{j=1}\iR_{2} \neq \iInv_j)$
%\end{itemize}
\end{enumerate}
\end{example}
