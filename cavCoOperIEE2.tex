...
\section{}
Due to  theorem \ref{consistentLemma}, the original problem of invariant checking boils down to  checking causal relation   some  causal relation between some $f\in invs$ and $r \in rs$, and different kinds of cuasal relation among hold in different cases which are splitted by comparison scheme. Notice that the comparison scheme is only determined by the number of rule parameters and formula parameters.

In order to know which kind of , we can select a special instance of the generalized symbolic case, and 
\forget{ Our solution
