...
\lyj{
Theorem \ref{consistentLemma} is our main weapon  to prove. Let us recall the proof goal set in Example \ref{example1}:  the mutual exclusion property holds for each reachable state of the mutual-exclusion protocol. %Let us recall example \ref{example1} and  \ref{example2}.
...
\end{enumerate}
\end{example}
Note that the above cases are mutually disjoint and their disjunction  is true. Besides, \lyj{$[3] \simeq_3^1 [4]$ and  they are both the special cases where $LR_{[1]}\ne 1$ and $LR_{[1]}\ne 2$. This is the reason why [4] is not needed to chosen to instantiate $\mathsf{crit}$.}
