...
For simplicity, a 
A parameterized formula(statement, rule, and protocol) is a function $f(x_1,x_2,...,x_n)$ from a list of  to such an object. Without lossing the generality, we require that parameters to instantiate a parameterized object are disjoint. For instance, $mutualInv(i,j)\equiv
...
Given a protocol $\mathcal{P}$ and a property set $F$ containing invariant formulas we want to verify, $\mathsf{invFinder}$ in Algorithm \ref{alg:invfinder} aims to find useful auxiliary invariants and causal relations which are capable of proving any element in $F$.
...
Recall that the aim of our policy is to compute groups of rule parameters to instantiate a parameterized rule into a set of concrete rules. Here $cf$ can be regarded as a special instance of some parameterized formula $f$ s.t. $cf=apply(f,idp(|apOfCForm(f)|))$.  Combination of  such a group of rule parameters with the actual parameters occurring the concrete invariant formula $cf$ will be a special instantiation of some case where we compare symbolic parameters in $r$ and those in $f$.
...
\begin{algorithm}
\caption{Computing quotient of $\mathsf{perms}_{m}^{n}$: $cmpSemiperm$ \label{alg:computeSemiPerms}}%\label{alg:invfinderII}
...

{
    ...
    $relItem\leftarrow (cr, inv, invRule_3,newInv)$\;
