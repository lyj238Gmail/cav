...
parameterized verification is that  safety properties, represented
...
model-checking techniques. In this paper, we proposes a novel
approach for parameterized verification of cache coherence
protocols. We first propose a general inductive model to formalize
...
generate the large number of sub-proofs with the help of the
Isabelle theorem prover because the proof patterns for the subgoals
are usually similar and modular. Then we show how to automatically
...
approach is demonstrated by case studies of some typical benchmarks.
\end{abstract}
%-------------------------------------------------------------------------
...
protocols play a key role in the modern parallel or distributed
computer systems. They are complex algorithms that must deal with
...
guaranteed by formal reasoning technique.
 ...
 Let $P$ be a parameterized system and
 $P(N)$ denotes its instance with the cardinality $N$.  $P(N)$ usually
consists of a small number (may be none) of heterogeneous processes,
plus a set of homogeneous processes $\{ P_i\, |\, 0 \le i \le N \}$.
When $N$ is small, verification of $P(N)$ w.r.t.\ some
invariants can be solved by applying model-checking.\footnote{
Due to high concurrency, a system with
only 4-5 nodes running a cache protocol may have a huge number of
possible executions. However, the practical systems normally consist of
hundred of nodes.}
However, we need to prove that the invariants hold in any
...
verification~\cite{Pnueli1996,BjÃ¸rner1997,Arons2001,Pnueli2001,Tiwari2001,Chou2004,Pandav2005,Lv2007,cubicle2011}.
...
parameterized systems.
 An abstract model of a parameterized protocol,  which consists of m + 1
...
\emph{invisible} as they are raw information, e.g., given in
...
Recently, cubicle has been developed for verifying safety properties
of parameterized systems \cite{cubicle2011}. cubicle  model-checks a
...
above invariants are still not enough to be used to derive a formal
proof in a theorem prover. First, they are not a parameterized (or
symbolic) formulas. Second, there are no causal relation between
invariants and rules, which are needed to explore fully to give a
formally inductive proof.

%An early work on flash, depends on the specific to FLASH, not
%generalized to

Theorem prover PVS  has been applied to the parameterized
verification of the   Stanford FLASH  cache protocol
\cite{Park1996PVS}. The proof relies on an abstraction function
which aggregates the implementation steps of each transaction into a
single atomic transaction in the specification. In fact, the
abstraction function maps an implementation model to a high-level
abstraction model, and the abstraction
 guaranteed that a property is proved at the abstract model implied that
 the property also holds at the implementation model. Crucial properties
of the protocol including data consistency are proved at the
abstract model. However, the design of abstraction function here is
heavily relied on people's special knowledge  on the FLASH protocol,
not easily  generalized to other protocols. Until now, there are not
generic and automatical abstraction method to the larger protocol
with the same scale like FLASH. This FLASH model in PVS has been
adapted many times in the later studies on FLASH protocols.
...
parameterized cache coherence protocols and prove their invariants.
 Besides, it is very difficult to obtain auxiliary invariants in proper form in order to
 give a rigorous proof in a theorem prover. Usually auxiliary invariants is derived from a finite small
 reference protocol model, which is ground formula, not a parameterized formula. At last, not only do we need
 invariants, but  also need a kind of causal relation between
  invariants and rules. Only do we have such causal relations, we
  can prove the invariants inductively.

%-------------------------------------------------------------------------
\subsection{Our contributions}\label{sec:overview}
%-------------------------------------------------------------------------
%To the best of our knowledge, there does not exist a generic
%mechanical theory formalized in a theorem prover to model cache coherence protocols and prove
%their invariants.
We aim to bridge the gap in this field by studying parameterized
verification of cache coherence protocols in a theorem prover like
Isabelle.

The main contribution of this paper is twofold. The first one is to
develop a theoretical result to give a general model to formalize
parameterized cache coherence protocols, and a general
proof principle to prove their invariants. The second is to introduce
an empirical approach to automate our modeling and proof methods in
Isabelle. We briefly discuss the novelties of our work below:
%
\begin{itemize}
...
fixed configuration size $100$, while we aim to verify that the invariants hold  for any
positive natural number that the parameter $NODE$ is set.

%=========================================
\section{A general model for formalizing cache coherence protocols}
%=========================================
According to the aforementioned discussion in Section~\ref{sec:informalAccounting},
firstly we define the variables used in protocols.
...
%type\_synonym paraStatement= nat $\Rightarrow$ statement\\
%primrec cat::statement $\Rightarrow$ statement
%$\Rightarrow$statement where\\
...
Expressions and formulas are coupled together to define. An
expression used in our work is can be simple, which is either a
...
parallel, \emph{not} in a sequential order. Our modelling language
is suitable to describe cache coherence protocols.
...
%A parameterized formula is just a function from a natural number to
%a formula.
%
%Similarly, a parameterized formula is a function from a parameter to
%a formula. We also define
% the $\mathsf{forall}$ and $\mathsf{exists}$ formulas.
%
%\begin{specification}
%
%type\_synonym paraFormula=nat $\Rightarrow$ formula\\
%
%fun forallForm::nat list $\Rightarrow$paraFormula $\Rightarrow$
%formula where\\
%\> oneAllForm: forallForm [i] forms = forms i| \\
%\>moreAllForm: forallForm (i\#j\#xs)  forms = \\
%\>andForm (forms i)
%(forallForm (j\#xs)  forms)\\
%
%
%fun exitsForm::nat list $\Rightarrow$paraFormula $\Rightarrow$
%formula where\\
%\> oneExForm:  exitsForm [i] forms = forms i|\\
%\>moreExForm:  exitsForm (i\#j\#xs)  forms = \\
%\>orForm (forms i)
%(forallForm (j\#xs)  forms)\\
%s
%\end{specification}
...
nat $\Rightarrow$ (nat $\Rightarrow$ bool) $\Rightarrow$bool\\
 ...
 where
exLessP N P $\equiv$ $\exists$i. ( i$\le$
 N $\wedge$ P i )\\
\\
 definition rules::
nat $\Rightarrow$ rule set
  where \\
\\
 rules N$\equiv$ $\{$
r. exLessP N ($\%$ i. r=crit   i) $\vee$ ~ exLessP N ($\%$ i. r=exit
i)  $\vee$\\
 exLessP N ($\%$
i. r=idle i)  ~$\vee$ exLessP N ($\%$
i.  r=try i)  $\}$\\
\\
...
In the above formalization, we first define the constants that are
used. Second we define parameterized rules $\mathsf{ try }$,
$\mathsf{ crit }$, $\mathsf{ exit }$, and
$\mathsf{ idle }$. In the third place, initialization of the
protocol is defined as a predicate $\mathsf{ mutualIni}$.
As its name indicates, the predicate $\mathsf{exLessP}~ N ~P$
says that there exists a natural number that is less than   $N$ and the property $P~i$ holds.
$\mathsf{rules}~N$ just defines the set of all transition rules used
in this work. The mutual exclusion property is defined as
  $\mathsf{mutualInv}$.
...
|\\
trans (  parallel  ( pair) sent2) s=  ( trans sent2 s) ((fst pair):=
expEval  (snd pair) s)
\end{specification}

Here we must point out the fact that the assignments are executed in parallel, therefore we always assign an
evaluation of an expression in the state $s$ to the corresponding
variable.
...
as invariant strengthening is well studied in the literature.
...
proving the single invariant $inv$, we need to prove any invariant
...
substIte: substExp  (iteForm f e1 e2)  asgns= (iteForm \\(substForm
f
asgns) (substExp e1  asgns) (substExp e2  asgns))|\\\\
...
Let $asgn\!=\![(v_1,e_1),\ldots,(v_n,e_n)]$ be an assignment to variable
$x_i$ by expression $e_i$, $\mathsf{substExp}~asgn~e$ ($\mathsf{substForm}~asgn~f$)
...
%
 The consistency lemma is the corner stone in our work. Not only does it provide   a strong weapon to prove the parameterized invariants
  in a parameterized model, but also    shows the strong hints to
automatically generate auxirially ground invariants and causal relations from a small reference protocol model. Furthermore, we can
 generalize the ground invariants and causal relations into parameterized invariants and causal relations which can be used for the proof of the parameterized invariants. Especially, the generalization process can be fully automated. Namely, we can almost get an formal proof In some theorem prover (e.g., Isabelle) automatically, which in turn is also can be checked machanically. %This divide and
 % conquer strategy of consistency lemma is the key to make the series of sub-proofs
 % to be automated because the proof patterns for the  subproofs  are similar
 % and modular. It is the similarity and modularity that allows us to
 % general proof templates and instantiate the template with different interpretations. to prove the main
...
to find. For example, a mutual exclusion property such as
 $\mathsf{mutualInv}~i~j$, which reflects the aim of a cache coherence protocol,
...
~~~~~~~~            by(cut$\_$tac  c2 d2£¬metis exit$\_$inv1.verify)\}\\
%~~~~           then have
...
Besides the simple mutual exclusion protocol, more experiments are
...
%scripts, and the simulation flow graph of one single node.
Table \ref{Summarization of experiment results} summarizes  our experiments below. Among the benchmarks, the
German protocol   was posted
 as
...
2000. German protocol is a moderate case. To the best of our
knowledge, few people but us give a complete proof to verify the
mutual exclusion  property of the German protocol
 in a theorem prover.

For the proof scripts of the above case studies, interesting readers
can refer to \cite{LiCache13} for details.
...
\hline
protocol &  ruleNum & invariantNum & lines & times  \\
\hline
simpMutual& 4& 5 & 2024 & 0'39.623" \\
\hline
MESI & 4& 3 & 1540 & 0'38.695"  \\
\hline
MOESI &  5& 3 &2942 & 0'37.874"  \\
\hline
 German-ish  & 6& 4 &3670 & 1'5.158"  \\
\hline
German & 13 & 48 & 76500 & 56'18.288"   \\
\hline
\end{tabular}
\end{table}

%%=========================================
%\section{ Case study on German protocol}
%%=========================================
%Our example of the mutual exclusion protocol is rather simple because the
%mutex variable $x$ is set $False$ when $n_i$ is set $C$, which means
%that the $i-$th client enters the critical section;  and $x$ is
%reset $True$ when $n_i$ is set $E$. This means that the $i-$th
%client leaves the critical section.  This is a typical lock
%mechanism that only one client is allowed to occupy the resource
%regardless of the operations of the client. This may cause less
%efficiency in cache coherence protocols. A more realistic protocols
%should allow multiple clients to share a memory with multiple copies
%in their caches for reading operations, and exclusive occupation
%must be guaranteed when a client want to do writing operations.
%German protocol, a directory-based protocol implement the
%aforementioned features, which was posted as a challenge to the
%formal verification community by Steven German in 2000.
%
%German protocol is a moderate case. In order to prove  the main
%mutual exclusion
%property $\forall i~j.i\neq j\longrightarrow cache[i]=exclusive\longrightarrow cache[j]=invalid$,
%we need another 47 invariants.
%%In order to save space, here we write the invariants in common logic formulas, not in our property specification logic.
%The proof script comprises 73863 lines, and the number of analyzed
%proof cases are 1313. The invariants and the detail proof scripts
%can be referred in~\cite{LiCache13}. In fact, German protocol is
%hard enough to demonstrate the power of our method. \emph{Neither}
%can the invariant sets  be found \emph{Nor} the Isabelle proof
%script can be completed solely by hand. At
% least until now, NO one but us give a pure proof to verify the mutual exclusion
...
In order to solve the hard parameterized
 verification problem, we automatically generate the proof script from    the general proof patterns drawn from the
 information computed in a specially small  instance of the
 protocol. The intuition behind our approach is that parameterized
 proof of a cache coherence protocol should share the same
 proof patterns which are suitable for a ground proof for a
  small protocol instance. Namely, the two proofs are "similar" in some sense. The core novelty of our work is to
  generalize the ground proof information to a general and parametric proof for
  the parameterized model. Isabelle's locale feature provides good
  supports  for us to implement the automatically generating
  procedures.
...
proof script contains about 76500 lines. Such a long proof task is beyond
a human's ability. %We have analyzed the German protocol using our
%approach. The proof script comprises 73,863 lines, and the number of
%analyzed proof cases is 1313. Due to page limit, the invariants and
%the detailed proof scripts can be found in~\cite{LiCache13}. In the
%future, we plan to perform more complex case studies, including the
%FLASH protocol.

  %Following
%generated proofs.
% Otherwise, just as we have mentioned, it is too tedious to finish so many proof
%cases  by only interactive proofs. Isabelle's locale feature is the
%key mechanism provided for us to implement the automatically
%generated proofs.
...
\end{document}
 ...
 to make proof generation easy and modular.  A command {\tt
  (is ?ANT1 $\longrightarrow$
 ?ANT2 $\longrightarrow$
 ?ANT3 $\longrightarrow$
  ?ANT4$\longrightarrow$
 ?ANT5 $\longrightarrow$
 ?P21 $\vee$ ?P22 $\vee$ ?P23 $\vee$ ?P24 $\vee$ ?P25)} is used to match the assertion {\tt i1  $\neq$ i2$\longrightarrow$ iR $\le$
 N $\longrightarrow$
i1 $\le$
 N$\longrightarrow$
i2 $\le$\\
 N$\longrightarrow$
iR = i1 $\longrightarrow$  invHoldForRule
(pInv i1 i2) (pRule iR)  (invariants
  N)}, and {\tt invHoldForRule
(pInv i1 i2) (pRule iR)  (invariants
  N) $\equiv $ invHoldForRule1
(pInv i1 i2) (pRule iR) $\vee$  invHoldForRule2
(pInv i1 i2) (pRule iR) $\vee$ invHoldForRule3
(pInv i1 i2) (pRule iR) (invariants N) $\vee$
  $\vee$ invHoldForRule4
(pInv i1 i2) (pRule iR) (invariants N) $\vee$ invHoldForRule5
(pInv i1 i2) (pRule iR) (invariants N) }.
