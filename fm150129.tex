...
moment there are no new invariants to be added either; for instance, let {\tt pRule=crit}, {\tt paras=[2]}, {\tt inv=aux~1 },
...
\begin{specification}
1\twoSpaces let newParaPolicy inv pr =\\
2\twoSpaces  let pNumOfInv=pNumOfInv inv in let pNumOfR=paraNumOfRule pr in\\
...
Now {\tt invFinder} requires restrictions with parameter
numbers of a rule and invariant such that {\tt pNumOfR $\le$
2} and {\tt pNumOfInv $\le$ 3}.  Due to symmetry property, the above sampling policies cover all parameterized cases by analysing
   the parameters in  invariant  $inv$ and  rule  $paraRule$ when $paraNumOfRule < 2$ or  $paraNumOfInv <
   2$.  Line 3 says that no parameters are needed to provide because {\tt pNumOfR = 0}. Line 4 says that a group of parameter [1] is only needed to instantiate the rule {\tt prule} when {\tt (pNumOfR = 1 ) AND (pNumOfInv  =0)}. Line 5 says that a group of parameters [1,2] is only needed to instantiate the rule {\tt prule} when {\tt (pNumOfR = 2 ) AND (pNumOfInv  =0)}. Two parameters are needed because the number of parameters of {\tt prule} is 2.  Line 6 says that two groups of parameters [[1],[2]] are needed to instantiate the rule {\tt prule} when {\tt (pNumOfR = 1 ) AND (pNumOfInv  =1)}. Other lines can be analysed similiarly.

Here we use our small example to illustrate the principle undelying our parameter instantiation policies. Let {\tt pr=crit},  {\tt inv=aux$_1$~1}. Because {\tt pNumOfInv inv=1} and {\tt paraNumOfRule pr=1}, we choose {\tt [[1],[2]]} at line 6 to instantiate {\tt pr} according to our policy.   Recall that the invariants searched by {\tt invFinder} are always in a normal form, whose parameters
  always start from 1, and increase one by one if there are more
  parameters. Here the sampling case {\tt inv} vs. {\tt pr 1} in the reference instance is symmetric to the parameterized case
    {\tt aux$_1$~i$_1$} vs. {\tt pRule~iR} where {\tt i$_1$=iR} in the parameterized instance, thus {\tt invHoldForRule$_1$~(aux$_1$~1)~(pr~1)} implies {\tt invHoldForRule$_1$~(aux$_1$~i$_1$~(pr~iR)} when {\tt i$_1$=iR}; the sampling case
    {\tt inv} vs. {\tt pr~2} in the reference instance is symmetric to  the parameterized case {\tt aux$_1$~i$_1$}
    vs. {pRule~iR} where {\tt i$_1$ $\ne$ iR} in the parameterized instance, thus {\tt invHoldForRule$_2$~(aux$_1$~1)~(pr~2)} implies {\tt invHoldForRule$_2$~(aux$_1$~i$_1$~(pr~iR)} when {\tt i$_1$$\ne$iR};

Consider another case: {\tt pr=crit},  {\tt inv=mutual$_1$ 1 2}. Because {\tt pNumOfInv inv=1} and {\tt paraNumOfRule pr=2}, we choose {\tt [[1]],[2],[3]} at line 7 to instantiate {\tt pr} according to our policy. In the view of symmetry,
     the sampling case {\tt in} vs {\tt pr~ 1} in the reference instance represents a
     parameterized case {\tt mutual$_1$~i$_1$~i$_2$} vs {\tt pRule~ iR} where {\tt i$_1$=iR} in the parameterized instance;
     the sampling case {\tt inv} and {\tt pr~2} in the reference instance represents a parameterized
     case {\tt mutual$_1$~i$_1$~i$_2$} and {\tt pr~iR} where {\tt i$_2$=iR} in the parameterized instance;  the sampling case {\tt inv} and
      {\tt pr~ 3} in the reference instance represents a parameterized case {\tt pInv~i$_1$~i$_2$} and {\tt pr~ iR}
      where neither {\tt i$_1$=iR} nor {\tt i$_2$=iR} in the parameterized instance.
...
%instantiate $pRule$ with parameter 2 at line 6, and the case where
%we instantiate paraRule with parameter 3 at line 7,
% since the causal relation $invHoldForRule_2$ will
%hold  in both  cases.

%Policies at line 5 and 6 are enough to handle many protocols, whose
%rules have at most one parameter.
Complex protocols such as Flash
...
choose a trade-off by using six groups of invariants  [1,2], [1,3], [2,3], [2,1], [3,1], [3,2]
to instantiate $pRule$. For Flash protocol, the invariants found by
{\tt invFinder} is enough to construct a parameterized proof. %Unlike policies (1) and (2), these parameter instantiation
...
%if $j$'s cache status is exclusive, then $i$ will be indicated by {\sf Home} to
%fetch the most recent copy from node $j$. The directory of Flash-system is set up and maintained in {\sf Home} node.
...
% index in this table is $5$ and $4$ respectively.}
 %The parameterized invariant is drawn from the ground invariant of $invFinder$. The index
