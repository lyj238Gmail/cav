...
%\vspace{-0.3cm}
\cai{Example 1 will be used throughout the paper.}
...
each formula in $fs$ holds at each inductive protocol rule cases. Note that the three kinds of inductive tactics can be done by (\cai{applied in?}) a theorem prover, which is the cornerstone of our work. Only after the theorem prover is told which one among the three kinds of  tactics is to be used, it can prove automatically. Without the fine-grained classification, the theorem prover cannot solve the proof tasks. In the procedure of automatic proof generation,  {\sf proofGen}  generates proof scripts which contain enough application of the three kinds of  tactics and guide the theorem prover to finish the proof.}
...
\lyj{
Theorem \ref{consistentLemma} is our main weapon (\cai{weapon seems strange... another word?})  to prove. Let us recall the proof goal set in Example \ref{example1}:  the mutual exclusion property holds for each reachable state of the mutual-exclusion protocol. %Let us recall example \ref{example1} and  \ref{example2}.
...
\end{enumerate}
\end{example}
Note that the above cases are mutually disjoint and their disjunction  is true (\cai{a tautology?}). Besides, \cai{[3] and [4] and both special cases where $LR_{[1]}\ne 1$ and $LR_{[1]}\ne 2$, and $[3] \simeq_3^1 [4]$}. This is the reason why [4] is not needed to \cai{be chosen} to instantiate $\mathsf{crit}$.
...
Let $cinv$  be a normalized concrete invariant w.r.t. a parameterized invariant $pinv$, $pr$ be a parameterized rule, $m$ be the number of actual parameters occurring in $cinv$, and $n$  be the number of formal parameters occurring in $pr$,  our policy is to compute a quotient of $\mathsf{perms}_{m}^{n}$, denoted as $\mathsf{cmpSemiperm}(m+n,n )$, and use \cai{its elements} as a group of parameters to instantiate $pr$ into a set $crs$ of concrete rules.\footnote{the details of computing $\mathsf{cmpSemiperm}(m+n,n )$ can be found in \cite{LiCache16}.} \lyj{ For instance, for the invariant $\negc (n[1]\eqc C \andc n[2]\eqc C)$ (or $\mathsf{mutualInv}(1,2)$), three groups of parameters [1], [2], [3] are used to instantiate $\mathsf{crit}$ respectively.} Each of the instantiation results (\caicomment{do instantiation results mean concrete rules? would be better to keep consistent.}) will be used to check which kind of  causal relation exists between the invariant and  each one of the resulting concrete rules. We will explain the checking work done by {\sf coreFinder}, which is illustrated in the following subsection.
...
After the second phase of the generalization, a flag of the symbolic cases and the according causal relations on rule $\mathsf{crit}$ and $\mathsf{mutualInv}$, which are transferred from Table \ref{table:groundCausalRelation} are shown as below:
...
\end{tabular}
\end{table}
where $r=\mathsf{crit}(\iR_1)$, and $f=\mathsf{mutualInv}(\iInv1,\iInv2)$.
%-------------------------------------------------------------------------
...
lemmas, and proofs. Readers can refer to \cite{LiCache16} for detailed illustration of the formal proof script. In this section, we focus on the generation of a lemma on the existence of causal relation between rule $\mathsf{crit}(\iR_1)$ and invariant $\mathsf{mutualInv}(\iInv_1,\iInv_2)$  basing on the aforementioned information listed in the Table. \ref{table:symbCausalRelation}. Firstly we show the lemma {\tt critVsinv$_1$} \footnote{Because each computed invariant will be referenced by an internal index $i$, and $\mathsf{mutualInv}$'s index is 1. Thus {\tt inv$_1$} is the  name for $\mathsf{mutualInv}$ to print.}  which is generated by {\sf proofGen}.
%{\tt critVsinv$_1$} and its proof in Isabelle in the {\tt mutualEx} protocol, is illustrated as follows:%An overview of the hierarchy  of the formal proof scripts of  is
...
scripts.  Experiment results are summarized in Table \ref{Summarization of experiment results}. Among them, German is mderate, and FLASH protocol is  industry-scale. 
