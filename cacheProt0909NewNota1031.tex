...
\center\begin{tabular}{|l|l|}
  ...
  $\mathbb{B}[f_1\vee f_2,s]\equiv \mathbb{B}[f_1,s]$  or $\mathbb{B}[f_2,s]$ \\
...
$symbolize(f)$ is called the simple symbolic representation. For instance, let $f$ be $\neg (n[1]\eqc C \andc n[2]\eqc C)$,   $symbolize(f )$ is $\neg (n[\iInv1]\eqc C \andc n[iInv2]\eqc C)$.   $symbolize(f )$ only shows the syntax effect of symbolic index replacement, we also need add  other model constraints with the symbolic transformation.
...
\item model constraint: $modelConstr(N,L) \equiv forallForm(pf,length(L))$, where $pf(i)=modelConstrI(N ,i)$.
\end{enumerate}
\end{definition}

For instance, $modelConstrI(N,  1)=\mathtt{iInv_1} \le N$; $modelConstr(N, [1,2])=\mathtt{iInv1} \le N \wedge \mathtt{iInv_2} \le N$.  Model constraints intuitively represents that any parameter index should be less than $N$, which is the  meaning of $N-$paramterized instance.

\begin{definition}
let $L$ be a permutation, 
\begin{enumerate}
\item difference constraint between parameter $\mathtt{iInv}_i$ and $\mathtt{iInv}_j$: $diff(i,j) \equiv \neg (\mathtt{iInv}_i = \mathtt{iInv}_j)$.
\item mutual difference constraint: $mutualDiff(L) \equiv \bigwedge S$, where $S=\{f. f=diff(i,j)~ and~ i\le len(L) ~and~ j \le len(L)~ and ~i < j\}$.
\end{enumerate}
\end{definition}
For instance, $diff(  1,2)=\neg(\mathtt{\iInv_1} = \mathtt{\iInv_2})$; $mutualDiff( [1,2])=\neg (\mathtt{iInv}_1 =\mathtt{ iInv}_2)$; $mutualDiff( [1,2,3])=\neg (\mathtt{iInv}_1 = \mathtt{iInv}_2) \wedge \neg (\mathtt{iInv}_1 = \mathtt{iInv}_3) \wedge \neg (\mathtt{iInv}_2 = \mathtt{iInv}_3)$. $mutualDiff$ emphasizes that  parameters of an formula   should be different from each other.
...
$generalizeS(\{f_1,..,f_m\},N)\equiv \{f'. sym(f',f_1,N) \orc ... \orc sym(f',f_m,N)\}$, where $f_i$ is a concrete normalized formula.
\end{definition}
...
\begin{definition}
Let $LR$ be a permutation s.t. $\mathsf{len}(LR)>0$, which represents a list of actual parameters to instantiate a rule,    let $LI$  be an identical permutation $\mathsf{len}(LI)>0$,  which  represents a list of actual parameters to instantiate a normalized invariant, we define:
\begin{enumerate}
...
\item symbolic partition decided by a $LRS$ and $LI$, where $LRS$ is a set of permutations with the same length: $partition(LRS,LI) \equiv existsForm(length(LRS),pf)$,  where $pf(i)= symbcase(LRS_i,LI)$
...
\begin{definition}
Let $LI$ and $LI$ are two permutations,\\
...
For instance, let $LI=[1,2]$, $LR=[2]$, then $symbolize'(invOnX_1(2),LI,LR)$ is $\neg(x=true \wedge n[\iInv2]=C)$. Let $LI=[1]$, $LR=[1,2]$, then $symbolize'(mutualInv(1,2),LI,LR)$ is $\neg(n[\iInv1]=C \wedge n[\iR2]=C)$.
%-------------------------------------------------------------------------
