...
type channel_type:
   record 
     channel_1: message_buf_type;
     channel_2: message_buf_type;
     channel_3: message_buf_type;
   end;
...
rule "Transfer message from `source' via `ch1'"
    dest = node[source].outchan.channel_1.msg.dest &
    node[source].outchan.channel_1.valid
  & !node[dest].inchan.channel_1.valid  ==>
begin
  node[dest].inchan.channel_1.msg := node[source].outchan.channel_1.msg;
  node[dest].inchan.channel_1.valid := node[source].outchan.channel_1.valid;
  clear node[source].outchan.channel_1;
endrule;

rule "Transfer message from `source' via `ch2'"
    dest = node[source].outchan.channel_2.msg.dest &
    node[source].outchan.channel_2.valid
  & !node[dest].inchan.channel_2.valid  ==>
begin
  node[dest].inchan.channel_2.msg := node[source].outchan.channel_2.msg;
  node[dest].inchan.channel_2.valid := node[source].outchan.channel_2.valid;
  clear node[source].outchan.channel_2;
endrule;


rule "Transfer message from `source' via `ch3'"
    dest = node[source].outchan.channel_3.msg.dest &
    node[source].outchan.channel_3.valid
  & !node[dest].inchan.channel_3.valid  ==>
begin
  node[dest].inchan.channel_3.msg := node[source].outchan.channel_3.msg;
  node[dest].inchan.channel_3.valid := node[source].outchan.channel_3.valid;
  clear node[source].outchan.channel_3;
...
rule "`client' generates new `req' for `addr'"
  ...
  & !node[client].outchan.channel_1.valid ==>
begin
    node[client].outchan.channel_1.msg.source := client;
	if addr = 0 then
      node[client].outchan.channel_1.msg.dest   := 0;
	else
      node[client].outchan.channel_1.msg.dest   := 1;
	endif;
	if req = req_read_shared then
	  node[client].outchan.channel_1.msg.op     := read_shared;
	elsif req = req_read_exclusive then
	  node[client].outchan.channel_1.msg.op     := read_exclusive;
	else
	  node[client].outchan.channel_1.msg.op     := req_upgrade;
	endif;
    node[client].outchan.channel_1.msg.addr   := addr;
    node[client].outchan.channel_1.valid := true;
    node[client].local_requests[addr] := true;
endrule;
endruleset;*/

ruleset client: node_id;  addr: addr_type do
rule "`client' generates new `req_read_shared' for `addr'"
...
rule "`client' generates new `req_read_exclusive' for `addr'"
...
rule "`client' generates new `req_req_upgrade' for `addr'"
...
rule "`client' accepts invalidate request" 
  ...
  & node[client].inchan.channel_2.msg.op = invalidate
  & node[client].remote_requests[addr].status = inactive ==>
begin
  node[client].remote_requests[addr].home   := node[client].inchan.channel_2.msg.source;
  node[client].remote_requests[addr].op     := node[client].inchan.channel_2.msg.op ;
  node[client].remote_requests[addr].status := pending;
...
rule "`client' prepares invalidate ack for `addr'"
    ...
    node[client].remote_requests[addr].status = completed
  & node[client].remote_requests[addr].op = invalidate
  & !node[client].outchan .channel_3.valid ==>
begin
    node[client].outchan .channel_3.msg.op     := invalidate_ack;
    node[client].outchan .channel_3.msg.source := client;
    node[client].outchan .channel_3.msg.dest   := node[client].remote_requests[addr].home;
    node[client].outchan .channel_3.msg.data   := node[client].remote_requests[addr].data;
    node[client].outchan .channel_3.msg.addr   := addr;
    node[client].outchan .channel_3.valid := true;
    clear node[client].remote_requests[addr];
endrule;
endruleset;
...
begin
    if node[client].inchan .channel_2.msg.op = grant_shared then
      node[client].cache[addr].data  := node[client].inchan .channel_2.msg.data;
      node[client].cache[addr].state := cache_shared;
    elsif node[client].inchan .channel_2.msg.op = grant_upgrade then
      node[client].cache[addr].state := cache_exclusive;
    elsif node[client].inchan .channel_2.msg.op = grant_exclusive then
      node[client].cache[addr].data  := node[client].inchan .channel_2.msg.data;
      node[client].cache[addr].state := cache_exclusive;
    endif;
    node[client].local_requests[addr] := false;
...
begin
  if node[home].inchan.channel_1.msg.op = req_upgrade & node[home].directory[addr][source] = cache_invalid then
    node[home].inchan.channel_1.msg.op := read_exclusive;
  endif;
  node[home].home_requests[addr].source := source;
  node[home].home_requests[addr].op := node[home].inchan.channel_1.msg.op;
/*1.
  if node[home].inchan.channel_1.msg.op = read_shared & node[home].directory[addr][home] = cache_shared 
  then
...
/*2.  elsif node[home].inchan.channel_1.msg.op = read_shared  & node[home].directory[addr][home] = cache_invalid
    ...
    node[home].home_requests[addr].status := completed;
/*3.  elsif node[home].inchan.channel_1.msg.op = read_shared &
	exists n:node_id do node[home].directory[addr][n] = cache_exclusive endexists then
    for n:node_id do
...
/*4.  elsif (node[home].inchan.channel_1.msg.op = req_upgrade) then
    for n: node_id do
  ...
  elsif (node[home].inchan.channel_1.msg.op = read_exclusive) then
    for n: node_id do
      if node[home].directory[addr][n] != cache_invalid then
        node[home].home_requests[addr].invalidate_list[n] := true;
	  else
...
rule "`home' accepts a request message read_shared_home_cache_shared" 
    node[home].inchan.channel_1.msg.op = read_shared & node[home].directory[addr][home] = cache_shared &
    addr = node[home].inchan.channel_1.msg.addr &
    source = node[home].inchan.channel_1.msg.source &
    node[home].inchan.channel_1.valid
  & node[home].home_requests[addr].status = inactive ==>
...
rule "`home' accepts a request message read_shared home_cache_invalid-forall_not_exclusive" 
    node[home].inchan.channel_1.msg.op = read_shared  & node[home].directory[addr][home] = cache_invalid &
...
ruleset home: node_id; addr: addr_type; source: node_id; n:node_id  do

rule "`home' accepts a request message_req_upgrade_exits_one_not_cache_invalid " 
    node[home].inchan.channel_1.msg.op = req_upgrade & 
    node[home].directory[addr][src] = cache_invalid &
    (!(src=n))&
    (!(node[home].directory[addr][n] = cache_invalid ))  &
...
rule "`home' accepts a request message req_upgrade_no_src_cache_invalid_exits_one_invalid" 
    (node[home].inchan.channel_1.msg.op = req_upgrade) & 
    (!(node[home].directory[addr][src] = cache_invalid ))&
     !(src =n)&
    ! (node[home].directory[addr][n] = cache_invalid )   & 
...
rule "`home' accepts a request message read_shared_home_invalid_exits_one_exclusive" 
    node[home].inchan.channel_1.msg.op = read_shared &  
