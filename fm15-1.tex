...
\begin{abstract}
%-------------------------------------------------------------------------
Parameterized verification of cache coherence protocols is important
...
concurrent system. The correctness of the reference instance  does
...
is the core in the field of parameterized verification is the
...
these theory contains the apparent circularity in reasoning and
...
theory themselves are not easy to be understood, and is needed to be
checked mechanically due to the soundness of them need to be
...
paraVerifier} are compose of two parts:  a finder {\tt invFinder}
and a proof generator \{tt proofGen}. In order to verify the that an
...
of clients is constructed, {\tt invFinder} tries to search
...
\item Our verification products are a list of intersting invariants
and a formal proof script. Our invariants are visible,
 which can charateize the semantical features of the protocol, and aid people to
   precisely
    understand the design of the protocol.
  Furthermore, a formal proof  can

\item Our work is both automatical and scalable.

Despite in an invariant set $invs$, and a rule $r$ in a set of
protocol rules $rs$, {\sf invFinder} works in a very simple way: it
tries to prove the existence of some kind of casual relations
between a rule $r$ and $inv$, then it tries to construct a new
invariant $inv'$ and adds it into the invariant set $invs$ if the
current proving step is blocked. At this time, it will analyze the
form of rule and invariant, then create new invariants. This
searching procedure stops until no new invariants are created. The
originality of our work lies the following aspects: (1) {\sf
...
verification of cache coherence protocols. In safety-critical
...
Being different from  the automatical  methods of checking
invariants of parameterized system, our method focuses on the aiding
of the people to understand  the correctness of protocol  by finding
auxially invariants. In our points of proof view, the auxialy should
...
Our method is scaleable. Some industry-level cases studies are
sucessfully finished in A striking result is that we generate a new
...
is in fact an abstract transaction model of Flash, and need prove
the correspondence between the abstract model and the original Flash
model, and prove the correctness of the protocol in the abstract
model, and subsequently derive the correctness of protocol in the
original model by the correspondence. New new auxiliary variables
like {\tt fwdSrc} are needed to introduce with deep human insight to
Flash. Later research on Flash must also rely on these auxiliary
variables for verification. Our proof is done for the original model
of Flash without abstraction. Other rather complex cache coherence
protocols are verified including German2004 and Godson-T.
 i
...
relation information between the current rule and invariants. This
...
13\twoSpaces  \twoSpaces      ((isNew newInv (newInvs@invs))=> (newInvs@[newInv], relItem : casRel)\\
...
$\mathsf{mutualInv}~2~1$ in symmetry view. In another word,
...
% \end{description}
 ...
 (a)  the numbers of the parameters in  invariant  $inv$ and  rule  $paraRule$, which are formulated by $paraNumOfInv$
  and $paraNumOfRule$; (b) the complexity of protocol itself.

Now {\tt invFinder} supports the parameter instantiation policies,
as sown below:

\begin{specification}
1\twoSpaces let newParaPolicy  paraNumOfInv  paraNumOfRule=\\
2\twoSpaces  (paraNumOfRule = 0 ) =>[()]\\
3\twoSpaces  |(paraNumOfRule = 1 ) AND (paraNumOfInv  =0)=>[(1)]\\
4\twoSpaces  |(paraNumOfRule = 2 ) AND (paraNumOfInv  =0)=>[(1,2)]\\
5\twoSpaces  |(paraNumOfRule = 1) AND (paraNumOfInv = 1) => [(1),(2)]\\
6\twoSpaces   |((paraNumOfRule =1) AND (paraNumOfInv =2 OR
paraNumOfInv =3))  =>
  [(1)],(2),(3)]\\
7\twoSpaces  |(paraNumOfRule = 2) AND (paraNumOfInv = 1) => [(1,2),(2,1),(3,2)]\\
8\twoSpaces   |     [(1,2),(1,3),(2,1),(2,3),(3,1),(3,2) ];\\
\end{specification}

Obviously, {\tt invFinder}  supports restrictions with parameter
numbers of a rule and invariant such that {\tt paraNumOfRule $\le$
2} and {\tt paraNumOfInv $\le$ 3}.
...
%-------------------------------------------------------------------------
\subsection{Automatical generation of Isabelle proof by {\tt proofGen}}
%-------------------------------------------------------------------------
...
% \centering %
 \vspace{-0.8cm}
\includegraphics[width=1.0\textwidth]{transform.eps}
%\vspace{-0.5cm}
 \caption{Two figures}
\label{fig:arch}
\end{figure}
...
parameterized ground invariants, and  from causal relations
 ...
 The   transformation
  ...
  invariants
  in the parameterized model. An internal implementation of this
  transformation is
...
"invariants N$\equiv$ {f. exTwoLessP N ($\lambda$ i j.  f = inv1 i
j)  $\vee$ exLessP N ($\lambda$ i.  f= inv2 i)  $\vee$ exTwoLessP N
($\lambda$ i j.  f = inv3 i j)  $\vee$ exLessP N ($\lambda$ i.  f=
inv4 i)  $\vee$ exTwoLessP N ($\lambda$ i j.  f = inv5 i j)   }"


\end{specification}
   For instance, consider $gInv_4\equiv \neg (x=true \wedge n[1]=C)$, we define
...
reasoning such as moreover and ultimately to do  case analysis on
...
implication from, then we fix a rule $r$ and $inv$, and have two
...
$r=r_0$. Line 7 again splits cases of $r$ into all possible cases
...
5\twoSpaces   have c1:r=r$_0$...$\vee$ex1P N
($\lambda$i.r=r$_1$~ i)...$\vee$ex2P N ($\lambda$i~j.r=r$_2$~i~j)...\\
...
7\twoSpaces \twoSpaces   have d1:inv=inv$_0$...$\vee$ex1P N
...
8\twoSpaces\twoSpaces moreover $\{$assume d1:  inv= inv$_0$\\
...
9\twoSpaces\twoSpaces moreover $\{$assume d1:  ex1P N~($\lambda$i.inv=inv$_{1}$~i).\\
\twoSpaces\twoSpaces ~  from d1 obtain i$_1$  where d2: inv=inv$_{1}$~ i$_1$~~by(cut\_tac d1,auto)\\
\twoSpaces\twoSpaces~  have invHoldForRule inv r (invariants N)
\twoSpaces\twoSpaces by(cut\_tac c1 d1,simp, rule r$_0$Vsinv$_1$)\}
\\

\twoSpaces\twoSpaces\twoSpaces\twoSpaces ...\\

10\twoSpaces\twoSpaces moreover $\{$assume d1:  ex3P N~($\lambda$i~j~k.inv=inv$_{3}$~i~j~k)\\
...
8  \twoSpaces   proof(simp, rule\_tac x="aux1 1 " in exI,auto)qed\\
9  \twoSpaces then have "invHoldForRule' (inv1 i1 i2) (crit iR)
(invariants
  N)"
by auto\}\\

10  moreover\{assume  b1:"iR=i2"\\
11 \twoSpaces have "invHoldForRule3' (inv1 i1 i2) (crit iRule) (invariants N)"\\
12 \twoSpaces   proof(simp, rule\_tac x="aux2 1 " in exI,auto)qed\\
 \twoSpaces then have "invHoldForRule' (inv1 i1 i2) (crit iR) (invariants
  N)"
by auto\}\\

13   moreover\{assume  b1:(iR $\ne$  i1 ¡Ä   iR $\ne$  i2)"\\
14 \twoSpaces have "invHoldForRule2' (inv1 i1 i2) (crit iR) (invariants N)"\\
15  \twoSpaces  proof(simp, rule\_tac x="aux2 1 " in exI,auto) qed\\
16 \twoSpaces then have "invHoldForRule' (inv1 i1 i2) (crit iR)
(invariants
  N)"
by auto\} \\

16ultimately show "invHoldForRule2' (inv1 i1 i2) (crit iR)
(invariants N)\\
17qed\\
\end{specification}
...
Here, we use Isar commands for calculation reasoning such as
moreover and ultimately to do the proof of all case analysis. From
the above extracts of the proof session of the lemma, we can see
that Isar proof has a similar structure as the textual one for the
case Add pos1 introduced in Sect. 4.5.
...
{\sf paraVerifier}. A detailed proof for a rule instance and an
