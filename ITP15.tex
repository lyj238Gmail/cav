...
Formula $\mathsf{substForm}~ f~ (\mathsf{statement2Assigns} ~S)$ can be regarded as the precondition of $f$ w.r.t. statement $S$. Namely, 
...
We can view the relation $\mathsf{invHoldForRule}~s~ f
r ~fs$ in two ways. Firstly it defines some causality relation
between $inv$, $r$, and $invs$.  $\mathsf{invHoldForRule}_1~s~f~ r$
  means that after rule $r$ is executed,
 $f$ will be true immediately;   $\mathsf{invHoldForRule}_2~s~f~ r$ specifies that
  the precondition of $f$ w.r.t. statement $S$ is $f$, intuitively this means that none of state variables in $f$ is
 changed, and the execution of statement $S$ does not affect the
 evaluation of $f$.
 $\mathsf{invHoldForRule}_3~s~f~ r~invs$ states
  that  there exists another invariant $f' \in invs$ such that
  the conjunction of the guard of $r$ and $f'$ implies the precondition.
...
$rs$, one of the following $\mathsf{invHoldForRule_1 }~s~inv~ r$,
$\mathsf{invHoldForRule_2 }~s~inv~ r$, $\mathsf{invHoldForRule_3 }~s~inv~
r~invs$ holds.
...
%this proof method}?
\section{Discussion}
In this section, we discuss some special features in the verification of cache coherence protocols. 
\begin{itemize}

\item Theoretically, all the invariant formulas in the formula set $fs$ can be used in one induction proof step in the induction approach. Namely another general relation   $\mathsf{invHoldForRule'}~s~ f~ r ~fs \equiv $
$(\mathsf{andList}~fs \wedge (\mathsf{pre}~ r)) \longrightarrow \mathsf{preCond} ~f ~(\mathsf{act}
~r)$ can be defined. Can we adopt the rule  $\mathsf{invHoldForRule'}$ as a general induction rule and rely Isabelle's automatical proof tool like 
{\sf metis} to finish the goal? Our experience gives an negative answer. For ITP tools like Isabelle, they are essentially interactive and guided by human. Despite the fact more powerful automatical proof tactics are developped,   it is still difficult for the tools to automatically solve the goal after applying the genral induction proof method  $\mathsf{invHoldForRule'}~ f~ r ~fs$. The main reason is the fact that the invariant formulas are in a parameterized form, which involves symbolic parameters $N$, and contain existence qualifiers over node indexes. For instance, even in the case study of simple $\mathsf{mutual}$ protocol, after applying rule $\mathsf{invHoldForRule'}~ f~ r ~fs$, the remianing goals cann't be solved by $\mathsf{auto}$ or $\mathsf{metis}$ commands. Let alone the moderate protocols like German and Flash protocols, which contain more rules and invariants. People need tell the ITP tool to selecet a proof rule
among $\mathsf{invHoldForRule}_{1-3}$. Especially,  a proper invariant formula $f'$ in $fs$ should be provided if we apply $\mathsf{invHoldForRule}_{3}$, and prove that $(f' \wedge (\mathsf{pre}~ r))$ implies the precondition of $f$ w.r.t.  $(\mathsf{act}
~r)$.

\item  The second problem is the complexity of the proof and the scale of our proof method. The complexity of proof mainly depends on then number of rules and invariants. Usually the more rules a protocol has, the more invariants will be derived. For a rule $r$ and aninvariant $f$,  in the simple $\mathsf{mutual}$ protocol can be shown  by the numbers of case nalysis: . In each case, the proof goal can be solved by choosing proper a proof method
among $\mathsf{invHoldForRule}_{1-3}$.
...
%task.
Due to space limitation, we do not discuss in detail how
...
high intelligence to inspect the protocol under study, while the
others are routine and straightforward. First we recall our paper
proof in section \ref{sec:provingInvSets}, %Recall that the consistency theorem is our main weapon to prove,
which requires proving two parts of obligations.
Among (1) and (2), proof of Part (2) is quite difficult. % because
%firstly we must do case analysis on the form of rule $r$, invariant
%$inv$ on which we choose proper proof commands.
%Let us recall our paper proof in Lemma \ref{consistentLemma},
Fixing $r=paraRule~iR$ and $inv=inv_i ~i_1~i_2$ provided that the
parameter numbers of $r$ and $inv_i$ is 2 respectively, we  need to
do case analysis on the parameters $iR$, $i_1$ and $i_2$  in order
to prove an lemma. There are three cases: (a) $iR=i_1$, (b) $iR =
i_2$, and (c) $iR \neq i_1$ and $iR \neq i_2$. If we can prove some
kind of causal relation holds between $inv$ and $r$ in all three
cases, then finish the proof. But the   causal relation, which  we
choose, depends on the actual instantiation of $paraRule$ and $inv$.
Here we note two points in our proof for an rule with only one
parameter and an invariant with two parameters: (i) we always follow
a proof pattern where we do case analysis on on the parameters $iR$,
$i_1$ and $i_2$, and then prove the  cases respectively; (ii) we
need tell the generator which causal relation should be chosen when
the actual instantiation of the $paraRule$ and $inv$ is given. (i)
is independent in the actual rules and invariants, but (2) is not
so.
...
In order to make our proof modular and hierarchy, we design our
theory in two levels which is shown in Fig. \ref{fig:arch}(b). In
the first level, a general lemma is proved at an abstract level,
relative to a set of assumptions. The proof strategy at this level
formalizes the general strategy outlined in (i). At the second
level, we instantiate the abstract lemma with actual rules and
invariants, and obtain a series of actual lemmas. The proofs at this
level depends on the actual rule and invariant themselves.
Isabelle's feature of $\mathsf{locale}/\mathsf{interpretation}$ can
support our proof
generation strategy well. %\vspace{-1.25cm}
...
\subsubsection{Locale to formalize a general lemma}
 Typically a locale
is a parameterized theory which can be repeatedly
 instantiated (or interpreted) in different contexts. Here a kind of locale
 is created according to the numbers  of rule and invariant.  In this case study for {\sf simpMutual},
 only two
 ...
 parameters. For instance,  {\sf pRule1VspInv2} is shown as below:
...
nat$\Rightarrow$formula   and iR::nat \\
and i1::nat and i2::nat  and N::nat\\
 % \\
 assumes a:  $\exists$ant0 cons0. pInv i1 i2=implyForm ant0 cons0 ~ and\\
 b: i1$\neq$i2$\longrightarrow$iR$\le$N$\longrightarrow$i1$\le$N$\longrightarrow$
 i2$\le$N$\longrightarrow$iR=i1$\longrightarrow$invHoldForRule
(pInv i1 i2)\\ (pRule iR)  (invariants   N)
  and\\
%\\
c: i1$\neq$i2$\longrightarrow$iR$\le$N$\longrightarrow$i1$\le$N$\longrightarrow$
i2$\le$N$\longrightarrow$iR=i2$\longrightarrow$invHoldForRule (pInv i1 i2)\\
 (pRule iR) (invariants  N)  and \\
%\\

d:i1$\neq$i2$\longrightarrow$iR$\le$N$\longrightarrow$i1$\le$N$\longrightarrow$
i2 $\le$ N$\longrightarrow$iR$\neq$i1$\longrightarrow$iR$\neq$i2$\longrightarrow$
invHoldForRule\\
 (pInv i1 i2)  (pRule iR)   (invariants
  N)\\
begin theorem verify:\\

 assumes a1:iR $\le$ N  and a2: i1 $\le$ N and a3:i2 $\le$ N  and a4:i1 $\neq$ i2\\

shows invHoldForRule (pInv i1 i2) (pRule iR)  (invariants N)\\
  (is ?P pInv i1 i2 pRule iR  invs)\\

  proof -\\



   have d2:iR=i1 $\vee$ iR=i2 $\vee$  ((iR $\neq$ i1) $\wedge$ (iR$\neq$i2))
~  by auto\\%\\

   moreover  $\{$assume e1: iR=i1 have ?P pInv i1 i2 pRule iR  invs\\
~       by (metis  a2 a3 a4 b e1)   $\}$\\%\\


   moreover  $\{$assume e1: iR = i2  have ?P pInv i1 i2 pRule iR  invs\\
...
$iR = i2$. With the  assumptions, we have a general theorem {\sf
verify}, which says that if $iR$ and $i1$ and $i2$ are all less than
or equal to $N$, then $\mathsf{invHoldForRule}~ (pInv ~i1~i2 )~
(pRule~ iR)$. In fact   the general proof in this locale is the
general pattern mentioned in (i), which prove a general theorem
$\mathsf{verify}$.

\vspace{-0.5cm} \subsubsection{Obtaining an actual theorem by
interpreting a locale with the aid of ground causal relation table}
%\vspace{-3cm}

 Note that $pRule$ and $pInv$ are both formally
symbolic parameters in this locale, any instantiation of
$\mathsf{pRule1VspInv2}$ by substituting $pRule$ and $pInv$  with
actual parameters such   as $idle$ and $inv_5$ will be an actual
lemma. In Isabelle, a mechanism called {\sf Interpretation}, as
shown below, is provided to support such an instantiation. For
instance, in interpretation  {\sf crit\_inv5}, we instantiate the
locale $\mathsf{pRule1VspInv2}$ with actual parameters
$\mathsf{crit}$ and $\mathsf{inv5}$ to formulate the proof
...
obligations,
 ...
 %different from that in $\mathsf{crit\_inv1}$.
 How does the generator create proper proof commands according to
the actual rule and invariants? Our solution to such a proof
generation  in {\sf interpretation} relies on the generalization of
the ground causal information in Table \ref{label-ground-causal
relation}. The intuition behind the proof generation can be
illustrated  as below by finding the correspondence of a
parameterized proof and a ground proof, which is shown as below.
...
\begin{specification}
 ...
 crit::nat $\Rightarrow$ rule inv5::nat $\Rightarrow$  nat$\Rightarrow$formula iR::nat
i1::nat i2::nat\\


proof(unfold pRule1VspInv2$\_$def)\\
show ($\exists$ant0 cons0. inv5 i1 i2 = implyForm ant0 cons0) $\wedge$\\
(i1 $\neq$ i2$\longrightarrow$ iR $\le$  N $\longrightarrow$ i1
$\le$  N $\longrightarrow$       i2 $\le$  N $\longrightarrow$      iR = i1 $\longrightarrow$invHoldForRule \\
(inv5 i1 i2) (crit iR) (invariants N))  $\wedge$\\
(i1 $\neq$i2$\longrightarrow$ iR $\le$N$\longrightarrow$i1 $\le$
 N $\longrightarrow$i2$\le$ N $\longrightarrow$iR =
 i2$\longrightarrow$ invHoldForRule (inv5\\
 i1 i2) (crit iR) (invariants N))\\$\wedge$(i1 $\neq$i2$
 \longrightarrow$iR$\le$
 N$\longrightarrow$i1$\le$N$\longrightarrow$i2$\le$N
 $\longrightarrow$iR$\neq$i1$\longrightarrow$iR$\neq$i2$\longrightarrow$invHoldForRule(inv5\\ i1 i2) (crit iR) (invariants
 N))    (is ?P1 $\wedge$ ?P2 $\wedge$ ?P3 $\wedge$ ?P4 )\\
...
%
    have b2:?P2  (is ?ANT1 $\longrightarrow$ ?ANT2 $\longrightarrow$ ?ANT3 $\longrightarrow$  ?ANT4$\longrightarrow$ ?ANT5 $\longrightarrow$\\
 ?P21 $\vee$ ?P22 $\vee$ ?P23 $\vee$ ?P24 $\vee$ ?P25)  \\
 proof(rule impI)+\\
assume c1: ?ANT1 and c2: ?ANT2 and c3: ?ANT3 and c4: ?ANT4and c5:?ANT5\\
from c1 c2 c3 c4 c5  have ?P23\\
~        apply( -,cut$\_$tac c3, simp)\\
~        apply(rule$\_$tac x=implyForm (eqn (IVar (Global x))(Const True))\\
...
~    then show ?P21 $\vee$ ?P22 $\vee$ ?P23 $\vee$ ?P24 $\vee$ ?P25 ~        by blast\\
    qed\\

have b3:?P3  (is ?ANT1 $\longrightarrow$?ANT2$\longrightarrow$?ANT3$\longrightarrow$?ANT4$\longrightarrow$\\
 ?ANT5 $\longrightarrow$ ?P21 $\vee$ ?P22 $\vee$ ?P23 $\vee$ ?P24 $\vee$ ?P25)\\
...\\
%    proof(rule impI)+\\
...
%    qed\\\\
     ...
     have b4:?P4 (is ?ANT1 $\longrightarrow$ ?ANT2 $\longrightarrow$ ANT3 $\longrightarrow$ ?ANT4 $\longrightarrow$ ?ANT5
     $\longrightarrow$\\
   ?ANT6 $\longrightarrow$ ?P21 $\vee$ ?P22 $\vee$ ?P23 $\vee$ ?P24 $\vee$ ?P25 )\\
...\\
%    proof(rule impI)+\\
...
%
%
  with b1 b2 b3 b4 show ?P1 $\wedge$ ?P2 $\wedge$  ?P3 $\wedge$  ?P4\\
%
~      by blast\\
   qed
%
...
$\mathsf{inv}_5$ in Table. \ref{label-ground-causal relation}. In
essence, we can regard such three lines as a ground proof of the
conclusion $\mathsf{invHoldForRule}~cinv~ cr~invs$ where
$cr=\mathsf{crit}~ciR$, $cinv=inv_5~1~2$, and $ciR \in \{1,2,3\}$ in
a ground model with size 3, which is shown as below.
\begin{description}
\item [(b-1)]$ciR=1$, let $cf'=\mathsf{inv}_4~2$, $cant'=\mathsf{eqn}~(\mathsf{IVar} ~
 (\mathsf{Global} ~x))~(\mathsf{Const}~True)$,  $ccons=\mathsf{neg} ~(\mathsf{eqn} ~(\mathsf{IVar}
 ~ (\mathsf{Para} ~ n ~2)) (\mathsf{Const}~ C))$,  $\mathsf{invHoldForRule3}~cinv~
 cr~cinvs$ holds
  because
...
%$\mathsf{inv}_4~2\in invs$, and $\mathsf{eqn}~(\mathsf{IVar}
%~(\mathsf{Global} ~x))~(\mathsf{Const}~True)$ is the guard of the
%rule $cr$.
\item [(b-2)] $ciR=2$, let $cf'=\mathsf{inv}_4~1$, $cant'=\mathsf{eqn}~(\mathsf{IVar} ~
...
%$\mathsf{invHoldForRule4}~cinv~ cr ~invs$ because $\mathsf{inv}_4~1
...
Now let us show the correspondence between (a-1) and (b-1). Other
case pairs have the similar correspondence.
\begin{description}
...
\vspace{-0.5cm} It is the correspondence which our generator fully
takes advantage of to generate the proof commands according to the
ground causal information stored in lines as shown in Table
...
% ~(\mathsf{neg}~
%\mathsf{(eqn}~ ( \mathsf{IVar}~ (\mathsf{Para} ~n~ i_1)) ~
%(\mathsf{Const}~ C)))$
$\mathsf{inv_4}\ i_1$ and $(\mathsf{eqn }~(\mathsf{IVar}~ (
\mathsf{Global}~ x) )$ are provided as $f'$ and $ant'$ to construct
the causal relation $\mathsf{invHoldForRule}$ respectively.
Parameter mapping $[1 \mapsto i_1]$ is needed to create the former
by replacing 1 in $f1'$ with $i_1$.
...
%theorem, two kinds of lemmas are prepared by using the {\sf
...
%\end{description}
%-------------------------------------------------------------------------
\vspace{-0.5cm} \subsubsection{Generating proofs for the main lemma}
\vspace{-0.5cm}
%-------------------------------------------------------------------------
At last, we discuss how to create automatically the proof for the main lemma, which depends
 on the applying the lemmas which are created in subsection \ref{sec:genOfIsabelleProof}. The proof of part (2) in the proof of the consistency theorem is based on the two levels of case analysis on the form of rules and the invariants.
...
%\end{itemize}
%is shown as below. The key points are as follows:
%\begin{itemize}
...
%\item The proof of part (1) in the proof of the consistency theorem is based on the case analysis on the form of the invariants.
...
next   show  $\forall$inv r. inv $\in$ invariants N$\longrightarrow$
 r $\in$rules N$\longrightarrow$invHoldForRule inv r (invariants N) \\
   ...
   proof((rule allI)+,(rule impI)+)\\
~~       fix inv r ~~     assume b1: inv ¡Ê invariants N  and b2:r ¡Ê rules N\\

~~       have c1:exLessP N ($\%$i.r=exit i)$\vee$exLessP N
($\%$i.r=idle i)
  $\vee$ exLessP N ($\%$i.r=crit i)$\vee$ \\
  ~~exLessP N ($\%$i.r=try i)
...
~~ moreover\\
$\{~~$assume c1:
 exLessP N ($\%$i.  rule= Exit i)\\
~~~~from c1 obtain iR where c2: iR $\le$
 N $\wedge$ r= Exit iR\\

~~ ~~          by (auto simp add: exLessP$\_$def)\\

~~ have c3:   exTwoLessP N ($\%$i j. inv=inv1 i j)$\vee$exLessP N
  ($\%$i.inv inv2 i)$\vee$ exTwoLessP N\\
  ~~ ($\%$ i j.inv=inv3 i j) $\vee$ exLessP N  ($\%$ i.  inv=inv4 i)$\vee$ exTwoLessP N($\%$
 i j. inv=inv5 i j)\\
